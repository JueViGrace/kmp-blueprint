package com.jvg.sample2.database.auth
...
import migrations.S2_session

interface AuthHelper : LocalDataSource {
    override val dbHelper: DbHelper<Sample2DB>

    suspend fun getSession(): S2_session?
    suspend fun createSession(session: S2_session)
    suspend fun updateActive(active: Boolean, id: String)
    suspend fun deleteSession()
}

class DefaultAuthHelper(override val dbHelper: DbHelper<Sample2DB>) : AuthHelper {
    override suspend fun getSession(): S2_session? {
    ...
    override suspend fun createSession(session: S2_session) {
